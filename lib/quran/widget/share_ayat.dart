// ✅ FULL SCREEN SHARE PAGE - With Warning & Display Options
import 'dart:io';
...
class AyahShareFullScreenPage extends StatefulWidget {
  final int ayahNumber;
  final SurahModel surah;
  final double fontSize;

  const AyahShareFullScreenPage({
    Key? key,
    required this.ayahNumber,
    required this.surah,
    required this.fontSize, String? ayahTextWithHarokat,
  }) : super(key: key);

  @override
  State<AyahShareFullScreenPage> createState() => _AyahShareFullScreenPageState();
}

class _AyahShareFullScreenPageState extends State<AyahShareFullScreenPage> {
  int _selectedDesign = 0;
  int _displayMode = 0; // 0: Keduanya, 1: Ayat saja, 2: Terjemahan saja
  ...
  // ✅ Deteksi ayat panjang
  void _checkContentLength() {
    final ayahText = widget.surah.getAyahText(widget.ayahNumber);
    final translation = widget.surah.getAyahTranslation(widget.ayahNumber);
    
    // Estimasi: ayat > 200 karakter ATAU terjemahan > 250 karakter
    setState(() {
      _showWarning = ayahText.length > 200 || translation.length > 250;
    });
  }
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                SizedBox(width: 12),
                Expanded(
                  child: Text('Gagal membagikan ayat: ${e.toString()}'),
                ),
              ],
            ),
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'Ayat panjang terdeteksi. Pilih mode tampilan:',
                                style: TextStyle(
                                  fontSize: 11,
  ...
  Widget _buildDisplayModeSelector() {
    return Container(
      ...
      child: Row(
        children: [
          _buildDisplayModeButton(0, 'Keduanya', Icons.view_agenda_rounded),
          _buildDisplayModeButton(1, 'Ayat', Icons.text_fields_rounded),
          _buildDisplayModeButton(2, 'Terjemahan', Icons.translate_rounded),
        ],
      ),
    );
  }
...
// ✅ STORY CARD - WITH DISPLAY MODE SUPPORT
class AyahShareStoryCard extends StatelessWidget {
  final int ayahNumber;
  final SurahModel surah;
  final double fontSize;
  final ShareDesignTheme theme;
  final int displayMode; // 0: Keduanya, 1: Ayat saja, 2: Terjemahan saja

  const AyahShareStoryCard({
    Key? key,
    required this.ayahNumber,
    required this.surah,
                          ...
                          if (displayMode == 0 || displayMode == 1)
                            Flexible(
                              child: Text(
                                ...
                                style: TextStyle(
                                  fontFamily: 'Utsmani',
                                  fontSize: displayMode == 1 ? 28 : 24, // Lebih besar jika solo
                                  height: 1.9,
                                  color: Colors.white,
                          ...
                          if (displayMode == 0 || displayMode == 2)
                            Flexible(
                              child: Text(
                                ...
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontSize: displayMode == 2 ? 16 : 14, // Lebih besar jika solo
                                  height: 1.7,
                                  color: Colors.white,
                                  fontWeight: FontWeight.w500,
                                  letterSpacing: 0.2,
